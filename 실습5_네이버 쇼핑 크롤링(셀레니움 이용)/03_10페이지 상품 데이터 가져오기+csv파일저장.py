...
#파일 생성 
# (C:\pratice_crolling\실습5_네이버 쇼핑 크롤링(셀레니움 이용)에 03_data.csv 파일 생성)
# 'w' -> 쓰기 모드로 연다. # 인코딩 타입은 CP949이다. # 줄바꿈 문자를 없앤다.
f = open(r"C:\pratice_crolling\실습5_네이버 쇼핑 크롤링(셀레니움 이용)\03_data.csv", 'w', encoding='CP949', newline='')
csvWriter = csv.writer(f)

for page in range(1, 11, 1): # 1~10페이지까지 크롤링 진행
    url = fr"https://search.shopping.naver.com/search/all?origQuery=%EA%B0%A4%EB%9F%AD%EC%8B%9C%2013&pagingIndex={page}&pagingSize=40&productSet=total&query=%EA%B0%A4%EB%9F%AD%EC%8B%9C%2013&sort=rel&timestamp=&viewType=list"
    browser.get(url)
